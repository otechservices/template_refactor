...
interface TestamentRegistrationProps {
  onBack: () => void;
  onComplete: (testamentData: any) => void;
}

export default function TestamentRegistration({ onBack, onComplete }: TestamentRegistrationProps) {
  ...
  const [testamentDate, setTestamentDate] = useState('');
  ...
  const handleSaveBeneficiary = () => {
    if (beneficiaryForm.lastname && beneficiaryForm.firstname && beneficiaryForm.relationship && beneficiaryForm.percentage) {
  ...
  const handleFinalSubmit = () => {
    ...
    setTimeout(() => {
      const testamentData = {
        id: Date.now(),
        code: `TEST-${String(Math.floor(Math.random() * 1000)).padStart(3, '0')}`,
        created_at: new Date().toISOString(),
        updated_at: new Date().toISOString(),
        status: 0,
        testament_type: testamentType,
        testament_date: testamentDate,
        ...
        steps: [
          {
            id: 1,
            step_name: 'Informations testateur',
            completed: true,
            data: testatorForm
          },
          {
            id: 2,
            step_name: 'Testament et témoins',
            completed: true,
            data: { testamentType, testamentDate, witnesses, secondNotary }
          },
          {
            id: 3,
            step_name: 'Documents',
  ...
  const getTotalPercentage = () => {
    return beneficiaries.reduce((total, ben) => total + parseFloat(ben.percentage || '0'), 0);
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-accent-100 to-accent-200">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              <div className="flex items-center">
                ...
                <div>
                  <h1 className="text-xl lg:text-2xl font-bold text-text-dark">Enregistrement de testament</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-6 py-6">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-accent-300 p-6">
          ...
          {/* Étape 2: Testament & Témoins */}
          {currentStep === 1 && (
            <div>
              {/* Informations du testament */}
              <div className="mb-6">
                <h3 className="text-base font-semibold text-text-dark mb-4">Informations du testament</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      type="date"
                      value={testamentDate}
                      onChange={(e) => setTestamentDate(e.target.value)}
                      className="w-full px-3 py-2 border border-accent-400 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 text-sm"
                      required
                    />
                  </div>
              ...
              <div className="flex justify-between">
                ...
                <button
                  ...
                  disabled={!testamentType || !testamentDate}
                  className="flex items-center px-6 py-2 bg-primary-500 text-white rounded-lg hover:bg-primary-600 transition-colors text-sm font-medium cursor-pointer disabled:opacity-50"
                >
                  Suivant
                  <i className="ri-arrow-right-line ml-2"></i>
                </button>
          ...
          {currentStep === 2 && (
            <div>
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-base font-semibold text-text-dark">Documents du testament</h3>
                <button
          ...
          {currentStep === 3 && (
            <div>
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h3 className="text-base font-semibold text-text-dark">Bénéficiaires du testament</h3>
                  <p className="text-sm text-text-muted mt-1">
                    Total attribué: {getTotalPercentage()}% / 100%
                  </p>
                </div>
                <button
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-accent-300">
                        ...
                        <th className="text-left py-3 px-4 text-sm font-medium text-text-dark">Pourcentage</th>
                        <th className="text-left py-3 px-4 text-sm font-medium text-text-dark">Contact</th>
                        <th className="text-left py-3 px-4 text-sm font-medium text-text-dark">Action</th>
                      </tr>
                    </thead>
                    <tbody>
                      {beneficiaries.map((beneficiary, index) => (
                        <tr key={beneficiary.id} className="border-b border-accent-200 hover:bg-accent-50">
                          ...
                          <td className="py-3 px-4 text-sm text-text-dark font-medium">{beneficiary.percentage}%</td>
              ...
              {getTotalPercentage() !== 100 && beneficiaries.length > 0 && (
                <div className="mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                  <div className="flex items-center">
                    <i className="ri-warning-line text-yellow-600 mr-2"></i>
                    <p className="text-sm text-yellow-800">
                      La répartition n'est pas complète. Total: {getTotalPercentage()}% / 100%
                    </p>
                  </div>
                </div>
              )}
          ...
          {/* Étape 5: Infos Exécuteur */}
          {currentStep === 4 && (
            <div>
              <p className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg text-sm text-blue-800">
                <strong>NB:</strong> Si pas d'exécuteur testamentaire, veuillez cliquer sur suivant sinon remplir tous les champs avec 
            ...
            <div>
              <div className="mb-6">
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 border border-accent-400 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 text-sm"
                  rows={4}
                  placeholder="Saisissez vos observations sur le testament..."
                  required
                />
              </div>
              ...
              <div className="mb-6">
                <label className="flex items-center">
                  ...
                  <span className="text-sm text-text-dark">
                    Ce testament doit-il faire l'objet d'une lecture urgente ?
                  </span>
                </label>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="border border-accent-200 rounded-lg p-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div>
                      <dt className="text-sm font-medium text-text-muted">Nom complet</dt>
                      <dd className="text-text-dark font-medium">{testatorForm.lastname} {testatorForm.firstname}</dd>
                    </div>
                    <div>
                    ...
                    <div>
                      <dt className="text-sm font-medium text-text-muted">Contact</dt>
                ...
                <div className="border border-accent-200 rounded-lg p-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <dt className="text-sm font-medium text-text-muted">Type de testament</dt>
                      <dd className="text-text-dark">{testamentType}</dd>
                    </div>
                    <div>
                      <dt className="text-sm font-medium text-text-muted">Date du testament</dt>
                      <dd className="text-text-dark">{formatDate(testamentDate)}</dd>
                    </div>
                  </div>
                </div>
                ...
                {witnesses.length > 0 && (
                  <div className="border border-accent-200 rounded-lg p-4">
                    <h3 className="text-base font-semibold text-text-dark mb-4 border-b border-accent-200 pb-2">
                      Témoins ({witnesses.length})
                    </h3>
                    <div className="space-y-3">
                      {witnesses.map((witness, index) => (
                        <div key={witness.id} className="p-3 bg-accent-50 rounded-lg">
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                            <div>
                              <dt className="text-xs font-medium text-text-muted">Nom complet</dt>
                              <dd className="text-sm text-text-dark font-medium">{witness.lastname} {witness.firstname}</dd>
                            </div>
                            <div>
                              <dt className="text-xs font-medium text-text-muted">Profession</dt>
                              <dd className="text-sm text-text-dark">{witness.job}</dd>
                            </div>
                            <div>
                              <dt className="text-xs font-medium text-text-muted">Contact</dt>
                              <dd className="text-sm text-text-dark">{witness.phone}</dd>
                            </div>
                          </div>
                        </div>
                      ))}
                ...
                {beneficiaries.length > 0 && (
                  <div className="border border-accent-200 rounded-lg p-4">
                    <h3 className="text-base font-semibold text-text-dark mb-4 border-b border-accent-200 pb-2">
                      Bénéficiaires ({beneficiaries.length}) - Total: {getTotalPercentage()}%
                    </h3>
                    <div className="space-y-3">
                      {beneficiaries.map((beneficiary, index) => (
                        <div key={beneficiary.id} className="p-3 bg-accent-50 rounded-lg">
                          <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
                            <div>
                              <dt className="text-xs font-medium text-text-muted">Nom complet</dt>
                              <dd className="text-sm text-text-dark font-medium">{beneficiary.lastname} {beneficiary.firstname}</dd>
                            </div>
                            <div>
                              <dt className="text-xs font-medium text-text-muted">Lien de parenté</dt>
                              <dd className="text-sm text-text-dark">{beneficiary.relationship}</dd>
                            </div>
                            <div>
                              <dt className="text-xs font-medium text-text-muted">Part</dt>
                              <dd className="text-sm text-text-dark font-medium">{beneficiary.percentage}%</dd>
                            </div>
                            <div>
                              <dt className="text-xs font-medium text-text-muted">Contact</dt>
                              <dd className="text-sm text-text-dark">{beneficiary.phone}</dd>
                            </div>
                          </div>
                        </div>
                      ))}
                ...
                {executorForm.lastname && (
                  <div className="border border-accent-200 rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      <div>
                        <dt className="text-sm font-medium text-text-muted">Nom complet</dt>
                        <dd className="text-text-dark font-medium">{executorForm.lastname} {executorForm.firstname}</dd>
                      </div>
                      <div>
                        <dt className="text-sm font-medium text-text-muted">Date de naissance</dt>
      ...
      {showBeneficiaryModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-medium text-text-dark">Ajouter un bénéficiaire</h3>
                <button 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div>
                  ...
                  <input
                    type="text"
                    value={beneficiaryForm.npi}
                    onChange={(e) => setBeneficiaryForm({...beneficiaryForm, npi: e.target.value})}
                    className="w-full px-3 py-2 border border-accent-400 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 text-sm"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setBeneficiaryForm({...beneficiaryForm, percentage: e.target.value})}
                    className="w-full px-3 py-2 border border-accent-400 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 text-sm"
                    required
                  />
                </div>
              ...
              <div className="flex justify-end gap-2">
                ...
                <button
                  ...
                  disabled={!beneficiaryForm.lastname || !beneficiaryForm.firstname || !beneficiaryForm.relationship || !beneficiaryForm.percentage}
                  className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors cursor-pointer disabled:opacity-50"
                >
